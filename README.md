...
4. 필요하면 모델(기본 `gemini-3.5-flash`), 언어(원문 `auto` → 대상 `Korean`), 배치 설정을 조정합니다. `모델 불러오기` 버튼을 누르면 입력한 API 키로 현재 사용 가능한 모델 목록을 조회해 드롭다운에 채워줍니다.
...
- **모델 선택** — 기본값은 `gemini-3.5-flash`입니다. Gemini 모델은 주기적으로 단종·교체되므로, `모델 불러오기` 버튼(CLI는 `--list-models`)으로 현재 계정에서 실제 사용 가능한 목록을 확인하고 고르세요. 품질보다 비용이 중요하면 `gemini-3.1-flash-lite` 같은 lite 계열을, 모델 단종 걱정 없이 쓰려면 자동으로 최신을 가리키는 `gemini-flash-latest`를 선택할 수 있습니다.
...
- 기본 모델을 `gemini-3.5-flash`로 변경 (`gemini-3.1-flash` 단종 대응)
- `모델 불러오기` 버튼 추가 — API 키로 현재 사용 가능한 모델 목록을 조회해 드롭다운에 채움 (CLI: `--list-models`)
- 저장된 설정에 단종 모델이 남아 있으면 기본 모델로 자동 교체
- 존재하지 않는 모델(404)로 요청 시 재시도를 반복하지 않고 즉시 안내 메시지와 함께 중단
